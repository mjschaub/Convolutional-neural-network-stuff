...
# <<#5>> Based on the val set performance, decide how many
...
# <<#1>> Use transforms.Normalize() with the right parameters to 
# make the data well conditioned (zero mean, std dev=1) for improved training.


# <<#2>> Try using transforms.RandomCrop() and/or transforms.RandomHorizontalFlip()
...
train_transform = transforms.Compose(
    [transforms.RandomCrop(32),transforms.RandomHorizontalFlip(),
	transforms.ToTensor(),transforms.Normalize((0.50787758, 0.48716971, 0.44120977), (0.26711263, 0.25646897, 0.27624937))])
...
class BaseNet(nn.Module):
    def __init__(self):
		...
		#conv2d params: (input_channels,output_channels,kernel_size,stride,padding)

        '''  This is the 31% accuracy stuff
        ...
        self.conv3 = nn.Conv2d(64, 128, 5,stride=1)#, stride=2, padding=1)
        ...
        self.fc_net = nn.Sequential(
            nn.Linear(128*1*1, TOTAL_CLASSES//2), #originally 16*5*5
            #nn.BatchNorm2d(50),
            nn.ReLU(inplace=True),
            nn.Linear(TOTAL_CLASSES//2, TOTAL_CLASSES),
            #nn.Linear(TOTAL_CLASSES//4, TOTAL_CLASSES),
            nn.BatchNorm1d(100),  #got accuracy to 30%
        )

    def forward(self, x):


        x = self.pool(F.relu(self.conv1(x))) #self.pool()
        # Output size = 28//2 x 28//2 = 14 x 14

        x = self.pool(F.relu(self.conv2(x))) #used to have pooling
        # Output size = 10//2 x 10//2 = 5 x 5

        #each 5 kernel size convolution reduces dimension by 4 on each width/height
        #each pool divides that even further by 2

        x = self.batch_norm(F.relu(self.conv3(x)))# used in 31% accuracy stuff
        #x = F.relu(self.conv3(x)) #used in 36% accuracy

        # See the CS231 link to understand why this is 16*5*5!
        # This will help you design your own deeper network
        x = x.view(-1, 128*1*1)#164 * 5 * 5) #16*5*5 originally
        x = self.fc_net(x)
